...
2. В объекте `SITE` замени:
   - `riddles` — последовательные загадки;
...
Сайт показывает шесть загадок подряд:

1. `Что последнее я готовил для своей китюськи?`
   Ответ — свинина с перцем и морковью, рис. Принимаются короткие ответы и перестановки вроде `свинина`, `свинина с рисом`, `рис со свининой`, `мясо с рисом`.
2. `36-е слово в моём последнем стишке тебе?`
   Ответ `мы` засчитывается в любом регистре.
3. `Первое слово, которое тебе придёт в голову.`
   Засчитываются варианты вроде `комбербутч`, `камбербутч`, `камбербуч`, `комбербуч`.
4. `На кого я похож?` — Добрыня Никитич или Леонардо ДиКаприо; принимаются распространённые варианты написания и одиночная опечатка в имени.
5. `Какое 25-е слово на картине?` — `сексуальный`.
6. `Закончи фразу: «Вот какая вот хуйня …»` — `собачка`.

Вступительный экран намекает, что появились новые загадки и сюрприз. Фото тайника хранится в `assets/surprise-closet.jpg`.
